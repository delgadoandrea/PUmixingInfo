...
{ 
  ...
  for (std::vector<std::string>::iterator it=names.begin();it!= names.end();++it)
  {
    ...
    // HepMCProduct
      ...
      flagHepMCProduct_ = true;      
      std::cout << "Inside HepMC Module" << std::endl; 
      InputTag tag;
      if (tags.size()>0) tag=tags[0];
      std::string label;
...
{ 
  ...
  if (flagHepMCProduct_){
    ...
    if (gotHepMCProduct){ 
      std::cout << "gotHepMCProduct" << std::endl;
      ...
      std::cout << "Please, check if the object <HepMCProduct> has been mixed by the MixingModule!" <<std::endl;
